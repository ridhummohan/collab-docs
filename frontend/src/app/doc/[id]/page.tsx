...
export default function DocPage() {
  ...
  const { content, editDocument, sendCursor } = useDocumentCollab(id as string, userId)
    ...
    async function load() {
      ...
      try {
        ...
        if (doc) {
          ...
          const share = sd.find((s: { document: { id: string } }) => s.document.id === id)
            ...
            : comments.map((c: {
                ...
                <div key={c.id} style={{ marginBottom: 16, padding: 12, background: "var(--bg-light)", borderRadius: 8 }}>
                  <div style={{ display: "flex", gap: 8, marginBottom: 4 }}>
                    <Avatar src={c.author.image} alt={c.author.name || "User"} size={32} />
                    <div>
